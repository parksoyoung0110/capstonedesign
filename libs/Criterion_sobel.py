...
def sobel_edge(img):
    ...
    return: [B, 1, H, W] edge magnitude
    """
    gray = TF.rgb_to_grayscale(img)  # [B, 1, H, W]
    gaussian_blur = T.GaussianBlur(kernel_size=3, sigma=0.5)
    img_blurred = gaussian_blur(gray)
    kernel_x = sobel_x.to(img.device)
    kernel_y = sobel_y.to(img.device)

    grad_x = F.conv2d(img_blurred, kernel_x, padding=1)
    grad_y = F.conv2d(img_blurred, kernel_y, padding=1)
...
class LossCriterion(nn.Module):
    ...
    def forward(self, tF, sF, cF, input_img=None, output_img=None):
        ...
        if input_img is not None and output_img is not None:
            edge_input = sobel_edge(input_img)
            edge_output = sobel_edge(output_img)
