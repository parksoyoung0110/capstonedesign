...
from libs.Criterion_laplacian import LossCriterion
...
def main():
    ...
    parser.add_argument("--outf", default="trainingOutput/laplacian")
    ...
    parser.add_argument("--edge_weight", type=float, default=40)
    ...
    loss_log_path = os.path.join(opt.outf, 'loss_log.txt')
    ...
    for iteration in range(1, opt.niter + 1):
        ...
        if iteration % 1000 == 0:
            with open(loss_log_path, 'a') as f:
                f.write('Iter: [%d/%d] Loss: %.4f content: %.4f style: %.4f edge: %.4f LR: %.6f\n' %
                    (iteration, opt.niter, loss.item(), contentLoss.item(), styleLoss.item(), edgeLoss.item(), optimizer.param_groups[0]['lr']))
