...
for i in range(NUM_VIEWS):
    print(k)
    view = meshutil.sample_view(18.0, 18.0)
    ...
    for y in range(height):
        for x in range(width):
            ...
            fv = uv[1] * (TEXTURE_SIZE - 1)

            # floor 기반 정수 좌표 및 보간 비율
            u0 = int(np.floor(fu))
            v0 = int(np.floor(fv))
            u1 = u0 + 1
            v1 = v0 + 1
            ...
            for (uu, vv, w) in [(u0, v0, w00), (u0, v1, w01), (u1, v0, w10), (u1, v1, w11)]:
                if 0 <= uu < TEXTURE_SIZE and 0 <= vv < TEXTURE_SIZE:
                    new_texture[uu, vv] += transfer_image[y, x] * alpha * w
                    alpha_accumulator[uu, vv] += alpha * w
