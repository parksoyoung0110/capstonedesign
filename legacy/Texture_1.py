...
for i in range(NUM_VIEWS):
    ...
    for y in range(height):
        for x in range(width):
            ...
            u = int(uv[0] * (TEXTURE_SIZE - 1))
            v = int((uv[1]) * (TEXTURE_SIZE - 1))  # flip Y if needed

            if 0 <= u < TEXTURE_SIZE and 0 <= v < TEXTURE_SIZE:
                new_texture[u, v] += transfer_image[y, x] * alpha
                alpha_accumulator[u, v] += alpha
